...
linearExp a n = L.foldl' (<*>) f1 $ replicate (fromIntegral n) a
...
findGenerator op' elems =
    ...
    L.find (\g -> let s = genOrderSet [] g g in length s == n) elems
  where
    n = length elems
...
newtype Z (a :: Nat)
  = Z { unZ :: Integer }
    deriving (Enum, Eq, Generic, Hashable, Ord)
$(makeWrapped ''Z)
...
newtype Poly a
  = Poly { unPoly :: [a] }
    deriving (Functor, Generic, Ord)

deriving instance (Hashable a, AGroup a) => Hashable (Poly a)
...
instance AGroup a => IsList (Poly a) where
    type Item (Poly a) = a
    fromList xs = stripZ $ Poly xs
    toList = unPoly
    ...
    lhs@(Poly p1) <*> rhs@(Poly p2) =
        let acc0 :: [a]
            ...
            foldFoo acc ((e1,d1),el2) =
                L.foldl' foldFooSub acc $ map ((e1,d1),) $ withIndex' el2
        in stripZ . Poly $ reverse $ L.foldl' foldFoo acc0 $ map (,p2) $ withIndex' p1
...
getCoeffPoly = toZ <$> (reflectCoeffPoly @p @n)
...
assert b str action = bool (error $ T.unpack str) action b
...
newtype FinPoly (p :: Nat) a
  = FinPoly { unFinPoly :: Poly a }
    deriving (Eq, Generic, Ord)

deriving instance (Hashable (Poly a), AGroup a) => Hashable (FinPoly p a)
...
invPolyEuclidian x = do
    ...
    let g' = case g of
           Poly [el] -> el
           _         -> error "bla"
    let res = mkFinPoly (fmap (<*> finv g') u)
